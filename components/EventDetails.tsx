        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
          {categories.map((cat, idx) => (
            <AnimatedSection key={idx} delay={idx * 0.1}>
              <div className="group bg-neutral-900/50 rounded-none border border-white/10 p-8 hover:border-tko-yellow/50 hover:-translate-y-1 hover:shadow-lg hover:shadow-black/30 transition-all duration-300 relative overflow-hidden hover:bg-neutral-800/50 h-full">
                <div className={`absolute top-0 left-0 w-full h-1 ${cat.color} transition-all duration-300 group-hover:h-1.5`}></div>
                <div className={`${cat.color} w-14 h-14 flex items-center justify-center shadow-lg shadow-black/50 mb-6 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300`}>
                  {cat.icon}
                </div>
             ...
             <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3">
                ...
                <div className="lg:col-span-2 p-10 lg:p-16 grid grid-cols-1 sm:grid-cols-2 gap-10">
                  {awards.map((award, idx) => (
                      <AnimatedSection key={idx} delay={idx * 0.1} direction="left">
